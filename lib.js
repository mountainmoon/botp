 ...
 *
 * @async
 * @return {Promise<string>} the one time password
 *
 * Arguments:
 *
 *  args
 ...
 *
 */
hotp.gen = async function(key, opt, getHmac) {
  key = key || '';
  opt = opt || {};
  var counter = opt.counter || 0;

  var h = await getHmac(key, intToBytes(counter));
 ...
 *
 * @async
 * @return {Promise<Object>} null if failure, { delta: # } on success
...
hotp.verify = async function(token, key, opt) {
  opt = opt || {};
  // https://github.com/guyht/notp/pull/36
  var window = opt.window >= 0 ? opt.window : 1;
  var counter = opt.counter || 0;
  ...
  for (var i = counter - window; i <= counter + window; ++i) {
    opt.counter = i;
    // Avoid timing attacks https://github.com/guyht/notp/pull/48
    if ((await this.gen(key, opt) ^ token) === 0) {
 ...
 *
 * @async
 * @return {Promsie<Object>} null if failure, { delta: # } on success
 * delta is the time step difference between the client and the server
 *
 * Arguments:
 *
...
totp.verify = async function(token, key, opt) {
  ...
  return await hotp.verify(token, key, opt);
};
